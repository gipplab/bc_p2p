...
func UploadPeer(runenv *runtime.RunEnv, bootstrap_addr string, netConfig network.Config) {
	...
	runenv.RecordMessage("SimLatency_ms: " + strings.TrimRight(fmt.Sprint(netConfig.Default.Latency), "ms"))
	runenv.RecordMessage("SimPeerBandwidth_bps: " + fmt.Sprint(netConfig.Default.Bandwidth))
	runenv.RecordMessage("SimJitter_ms: " + strings.TrimRight(fmt.Sprint(netConfig.Default.Jitter), "ms"))
	runenv.RecordMessage("SimLoss_pct: " + fmt.Sprint(netConfig.Default.Loss))
	runenv.RecordMessage("SimCorrupt_pct: " + fmt.Sprint(netConfig.Default.Corrupt))
	runenv.RecordMessage("S2CheckDuration_s: " + strings.TrimRight((s2CheckEndTime.Sub(s2CheckStartTime)).String(), "s"))
	runenv.RecordMessage("OriginalityRatio_RO: " + fmt.Sprint(float32(len(unseenHashes))/float32(len(combinations))))
	runenv.RecordMessage("UploadedOriginalCombinations: " + fmt.Sprint(len(originalCombinations)))
	runenv.RecordMessage("DhtUploadDuration_s: " + strings.TrimRight((uploadEndTime.Sub(uploadStartTime)).String(), "s"))
	runenv.RecordMessage("DhtQueryDuration_ms: " + strings.TrimRight((dhtQueryEndTime.Sub(dhtQueryStartTime)).String(), "ms"))
